'use client';

import { useRouter } from 'next/navigation';
import style from './Card.module.css';
import Image from 'next/image';

const Card = ({ product }) => {
  const router = useRouter();

  const handleClick = (id) => {
    router.push(`./products/${id}`);
  };

  const defaultImagePath =
    'data:image/png;base64,';

  return (
    <div className={`${style.card} dark:border dark:border-[#0ea5e9] `}>
      <div
        className={`w-full mb-[10px] dark:bg-[#1E293B] h-[100%] rounded-md ${style.image_wrapper}`}
      >
        <Image
          width={300}
          height={200}
          alt={product.title}
          src={product?.images?.[0] || defaultImagePath}
        />
      </div>
      {/*  width: 100%;
  height: 100%;
  margin-bottom: 10px;
  overflow: hidden;*/}
      <div className={style.card_desc_wrapper}>
        <h1 className='text-base text-center mt-[10px] mb-[30px] dark:text-[#E2E8F0]'>
          {product.title}
        </h1>
        <div className={style.price_wrapper}>
          <p className='dark:text-[#E2E8F0] text-center mb-[20px] text-black'>
            {product.price}$
          </p>
          <button
            onClick={() => handleClick(product.id)}
            type='button'
            className={`${style.button} bg-white dark:bg-[#E2E8F0]`}
          >
            More
          </button>
          <button
            onClick={() => handleClick(product.id)}
            type='button'
            className={`${style.button} bg-white dark:bg-[#E2E8F0]`}
          >
            Delete
          </button>
        </div>
      </div>
    </div>
  );
};

export default Card;
